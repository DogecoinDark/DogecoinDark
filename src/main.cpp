...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // DogecoinDark: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // DogecoinDark: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// DogecoinDark: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes DogecoinDark fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // DogecoinDark: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // DogecoinDark: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // DogecoinDark: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // DogecoinDark: track money supply and mint amount info
...
// DogecoinDark: total coin age spent in transaction, in the unit of coin-days.
...
// DogecoinDark: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // DogecoinDark: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // DogecoinDark: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // DogecoinDark: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // DogecoinDark: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // DogecoinDark: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // DogecoinDark: check proof-of-stake
    ...
    // DogecoinDark: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // DogecoinDark: getblocks may not obtain the ancestor block rejected
    ...
    // DogecoinDark: if responsible for sync-checkpoint send it
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "DOGECOINDARK";
        CTransaction txNew;
        txNew.nTime = 1409847336;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1409847336;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3816805;
        if(fTestNet)
        {
            block.nNonce   = 3816805;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xba399fd089250c58f34d67fdcdab7209cf8beca9729cb2b40b1c2fb29b6b7562"));
        ...
        // DogecoinDark: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // DogecoinDark: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // DogecoinDark: tell downloading node about the latest block if it's
